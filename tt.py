...
def print_bot_move(move, board_size):
    if move.is_pass:
        move_str = 'passes'
    elif move.is_resign:
        move_str = 'resign'
    else:
        move_str =(move.point.col , board_size-move.point.row+1)
    return move_str
...
def point_to_point(point, board_size):
    col = point[0]
    row = board_size - point[1] +1
    return Point(row=row, col=col)
...
def main():
    ...
    bot = MCTSAgent(3, temperature=5.0)
    ...
    while not game.is_over():
        ...
        if game.next_player == Player.black:
            ...
            for r in range(0, game.board.num_rows + 1):
                for c in range(0, game.board.num_cols + 1):
                    pp = (r, c)
                    ppp = point_to_point(pp, board_size)
                    if game.board._grid.get(ppp) is None:
                        ui.remove(pp)
            # human_move = input('-- ')
            ...
            for event in pygame.event.get():
                if event.type == pygame.MOUSEBUTTONDOWN:
                    if event.button == 1 and ui.outline.collidepoint(event.pos):
                        ...
                        p = (x, y)
                        point = point_to_point(p, board_size)
            ...
            # point, move = move_by_agent(ui, bot, game)
            for r in range(0, game.board.num_rows + 1):
                for c in range(0, game.board.num_cols + 1):
                    pp = (r, c)
                    ppp = point_to_point(pp, board_size)
            ...
            bot_move = print_bot_move(move, board_size)
            ui.draw(bot_move, "WHITE")
